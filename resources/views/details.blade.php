        ...
        <div class="col-lg-5">
          ...
          <div class="product-single__addtolinks">
            ...
            @php $rowId = Cart::instance('wishlist')->content()->where('id', $product->id)->first()->rowId; @endphp
              <form method="POST" action="{{ route('wishlist.removeItem', ['rowId' => $rowId]) }}" id="removeItem">
                @csrf
                @method('DELETE')
                <a href="javascript:void(0)" onclick="document.getElementById('removeItem').submit();" class="menu-link menu-link_us-s add-to-wishlist filled-heart"><svg width="16" height="16" viewBox="0 0 20 20"
            ...
            @else
              <form method="POST" action="{{ route('wishlist.add') }}" id="addtowishlist">
                @csrf
                <input type="hidden" name="id" value="{{ $product->id }}">
                <input type="hidden" name="name" value="{{ $product->name }}">
                <input type="hidden" name="price" value="{{ $product->sale_price == '' ? $product->regular_price : $product->sale_price }}">
                <input type="hidden" name="quantity" value="{{ $product->id }}">
                <a href="javascript:void(0)" onclick="document.getElementById('addtowishlist').submit();" class="menu-link menu-link_us-s add-to-wishlist"><svg width="16" height="16" viewBox="0 0 20 20"
                    fill="none" xmlns="http://www.w3.org/2000/svg">
                    <use href="#icon_heart" />
                  ...
                  <a href="details.html">
                    <img loading="lazy" src='{{ asset("uploads/products/$rproduct->image") }}' width="330" height="400"
                      ...
                      @foreach(explode(',', $rproduct->images) as $rgimg)
                        <img loading="lazy" src='{{ asset("uploads/products/$rgimg") }}' width="330" height="400"
                  ...
                  <div class="product-card__price d-flex">
                    <span class="money price">
                      ...
                      @if(Cart::instance('wishlist')->content()->where('id', $rproduct->id)->count() > 0)
                        @php $rowId = Cart::instance('wishlist')->content()->where('id', $rproduct->id)->first()->rowId; @endphp
