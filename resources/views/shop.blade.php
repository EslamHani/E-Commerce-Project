                ...
                @foreach($categories as $category)
                    <li class="list-item">
                      <span class="menu-link py-1">
                        ...
                        @if(in_array($category->id, explode(',', $f_categories))) checked="checked" @endif class="chk-category">{{ $category->name }}
                      </span>
        ...
        <div class="accordion" id="brand-filters">
          <div class="accordion-item mb-4 pb-3">
            ...
            <div id="accordion-filter-brand" class="accordion-collapse collapse show border-0"
              ...
              <div class="search-field px-0 pb-0 pt-3 category_list">
                  <ul class="list list-inline mb-0 brand-list">
                    @foreach($brands as $brand)
                    <li class="list-item">
                      <span class="menu-link py-1">
                        <input type="checkbox" name="brands" value="{{ $brand->id }}"
                        @if(in_array($brand->id, explode(',', $f_brands))) checked="checked" @endif class="chk-brand">{{ $brand->name }}
                      </span>
                      <span class="text-right float-end">
      ...
      <div class="shop-list flex-grow-1">
        ...
        <div class="d-flex justify-content-between mb-4 pb-md-2">
          ...
          <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0" aria-label="Page Size"
              name="pagesize" id="pagesize" style="margin-right: 10px;">
              <option value="12" @selected($size == 12)>Show</option>
              <option value="24" @selected($size == 24)>24</option>
              <option value="48" @selected($size == 48)>48</option>
              <option value="96" @selected($size == 96)>96</option>
            </select>
            ...
            @foreach($products as $product)
                ...
                <div class="product-card-wrapper">
                    ...
                    <div class="pc__img-wrapper">
                        ...
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                            <a href="{{ route('shop.product.details', ['slug' => $product->slug]) }}"><img loading="lazy" src='{{ asset("uploads/products/$product->image") }}' width="330"
                                height="400" alt="{{ $product->name }}" class="pc__img"></a>
                            </div>
                            <div class="swiper-slide">
                            @foreach(explode(',', $product->images) as $gimg)
                                <a href="{{ route('shop.product.details', ['slug' => $product->slug]) }}"><img loading="lazy" src='{{ asset("uploads/products/$gimg") }}'
                    ...
                    <div class="pc__info position-relative">
                        ...
                        <h6 class="pc__title"><a href="{{ route('shop.product.details', ['slug' => $product->slug]) }}">{{ $product->name }}</a></h6>
                        ...
                        @php $rowId = Cart::instance('wishlist')->content()->where('id', $product->id)->first()->rowId; @endphp
                        <form method="POST" action="{{ route('wishlist.removeItem', ['rowId' => $rowId]) }}">
                          @csrf
                          @method('DELETE')
...
<form id="frmfilter" method="GET" action="{{ route('shop.index') }}">
  ...
  <input type="hidden" name="brandsname" id="hdnBrands">
